...
use crate::hints::{hide_hints, filter_hints};
use crate::inputs::{executor, mouse};
use crate::configs;
...
// 全局键盘状态
pub static KEYBOARD_STATE: Lazy<Mutex<KeyboardState>> = Lazy::new(|| {
...
static KEY_RIGHT_SIDE_MAP: Lazy<std::collections::HashMap<&str, &str>> = Lazy::new(|| {
    ...
    map.insert("A", "S"); map.insert("B", "N"); map.insert("C", "V");
    map.insert("D", "F"); map.insert("E", "R"); map.insert("F", "G");
    map.insert("G", "H"); map.insert("H", "J"); map.insert("I", "O");
    map.insert("J", "K"); map.insert("K", "L"); map.insert("L", ";");
    map.insert("M", ","); map.insert("N", "M"); map.insert("O", "P");
    map.insert("P", "["); map.insert("Q", "W"); map.insert("R", "T");
    map.insert("S", "D"); map.insert("T", "Y"); map.insert("U", "I");
    map.insert("V", "B"); map.insert("W", "E"); map.insert("X", "C");
    map.insert("Y", "U"); map.insert("Z", "X");
    map.insert("1", "2"); map.insert("2", "3"); map.insert("3", "4");
    map.insert("4", "5"); map.insert("5", "6"); map.insert("6", "7");
    map.insert("7", "8"); map.insert("8", "9"); map.insert("9", "0");
    map.insert("0", "-");
    map
});
...
static KEY_LEFT_SIDE_MAP: Lazy<std::collections::HashMap<&str, &str>> = Lazy::new(|| {
    let mut map = std::collections::HashMap::new();
    map.insert("W", "Q"); map.insert("E", "W"); map.insert("R", "E");
    map.insert("T", "R"); map.insert("Y", "T"); map.insert("U", "Y");
    map.insert("I", "U"); map.insert("O", "I"); map.insert("P", "O");
    map.insert("[", "P");
    map.insert("S", "A"); map.insert("D", "S"); map.insert("F", "D");
    map.insert("G", "F"); map.insert("H", "G"); map.insert("J", "H");
    map.insert("K", "J"); map.insert("L", "K"); map.insert(";", "L"); 
    map.insert("X", "Z"); map.insert("C", "X"); map.insert("V", "C");
    map.insert("B", "V"); map.insert("N", "M"); map.insert("M", ",");
    map.insert("1", "`"); map.insert("2", "1"); map.insert("3", "2");
    map.insert("4", "3"); map.insert("5", "4"); map.insert("6", "5");
    map.insert("7", "6"); map.insert("8", "7"); map.insert("9", "8");
    map.insert("0", "9"); map.insert("-", "0");
    map
});
...
fn is_right_key_of(key: &str, last_key: &str) -> bool {
    KEY_RIGHT_SIDE_MAP.get(last_key).map_or(false, |&right_key| right_key == key)
}

// 检查是否是末尾hint键的左侧键
fn is_left_key_of(key: &str, last_key: &str) -> bool {
    KEY_LEFT_SIDE_MAP.get(last_key).map_or(false, |&left_key| left_key == key)
}
...
pub fn handle_keyboard_event(app_handle: &tauri::AppHandle, key: &str, is_down: bool) -> bool {
    ...
    println!("[键盘事件] 按键: {}, 状态: {}, is_holding:{}", key, if is_down { "按下" } else { "释放" }, state.final_hint_key_hold);

    let config = configs::get_config().expect("配置未初始化");
    let keybindings = &config.keybindings;
    let modifier_keys = configs::keybindings::MODIFIERS.clone();
    ...
    // 处理按键按下
    if is_down {   
        let propagation_modifier: &Vec<String> = &config.keyboard.propagation_modifier;
        ...
        if !state.final_hint_key_hold {
            ...
            if key.len() == 1 {
                ...
                if state.pressed_hint_keys.clone().unwrap().is_empty() {
                    ...
                    if start_by_extra {
                        ...
                        start_by_charset = char_in_charset(key_char, hint_config.charsets[0].as_slice());
                        if start_by_charset {
                    ...
                    if start_by_extra || start_by_charset {
                        ...
                        if state.hint_length == 1 {
                            ...
                            current_key = config.keyboard.get_key_by_name(configs::keyboard::HINT_KEY_NAME).unwrap();
                                    }
            ...
            let cmd_key = configs::keybindings::GLOBAL_KEY_DOWN_KEYBINDINGS.clone();
            for (cmd, keys) in cmd_key {
                if key_in_keys(current_key, &keys) {
                    println!("[keyboard] global_key_down cmd:{} triggered by key: {}", cmd, key);
                    let mut executor = executor::Executor::new(app_handle, &config, &mut state);
        ...
        if state.final_hint_key_hold {
            ...
            let cmd_key = configs::keybindings::AT_HINT_KEYBINDINGS.clone();
            let mut current_key = key;
            if let Some(last_key) = state.final_hint_key.as_ref() {
                ...
                if current_key == last_key {
                    ...
                    current_key = config.keyboard.get_key_by_name(configs::keyboard::HINT_RIGHT_KEY_NAME).unwrap();
                } else if is_left_key_of(key, last_key) {
                    current_key = config.keyboard.get_key_by_name(configs::keyboard::HINT_LEFT_KEY_NAME).unwrap();
                }
            }
            for (cmd, keys) in cmd_key {
                if key_in_keys(current_key, &keys) {
                    println!("[keyboard] at_hint cmd:{} triggered by key: {}", cmd, current_key);
                    let mut executor = executor::Executor::new(app_handle, &config, &mut state);
        ...
        let cmd_key = configs::keybindings::GLOBAL_KEY_UP_KEYBINDINGS.clone();
        for (cmd, keys) in cmd_key {
            if key_in_keys(current_key, &keys) {
                println!("[keyboard] global_key_up cmd:{} triggered by key: {}", cmd, current_key);
                let mut executor = executor::Executor::new(app_handle, &config, &mut state);
